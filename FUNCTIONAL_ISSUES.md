...
| 跨维度（性能/可用性/边界/兼容性/可维护性/完整性） | 3 | 7 | 10 | 4 | 25（含 2 项设计说明） |
| **合计（去重后）** | **8** | **25** | **37** | **11** | **81** |
...
#### ℹ️ X-11｜`BAIZE_SANDBOX_MAX_CONTAINERS=10` 全局共享，容器多会话复用【设计说明，非缺陷】
- **位置**：`pentest/container_registry.py:44,51`
- **说明**：容器设计为可被多个会话共用的工具运行环境，而非与单个会话绑定。全局共享上限是合理的资源管理策略——容器只负责提供工具执行环境，不持有会话状态。按 session 隔离容器反而会浪费资源（每个会话独占一个完整容器），与"容器即工具运行时"的设计理念相悖。
...
| X-06 ~ X-08 | 部署与运维 | 部署 |
| X-11 | ~~容器并发按 session 隔离~~（设计说明，非缺陷） | 执行器 |
...
*报告结束。本次评估共识别 81 项功能缺陷（去重后），其中严重 8 项、高 25 项、中 37 项、低 11 项，另含 4 项设计说明（X-03、X-05、X-11、F-17）。所有发现均基于只读源码审查，未修改任何文件。*
