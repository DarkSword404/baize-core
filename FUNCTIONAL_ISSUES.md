...
| 前端（UI/交互/工程化/状态管理） | 3 | 8 | 6 | 3 | 20 |
| 跨维度（性能/可用性/边界/兼容性/可维护性/完整性） | 4 | 7 | 11 | 4 | 26 |
| **合计（去重后）** | **9** | **26** | **38** | **11** | **84** |
...
5. **功能完整性缺口**：无 API 限流、无 RBAC、报告无 PDF/Word 导出、无国际化、无多用户协同。
...
#### 🟠 F-17｜无国际化框架，UI 文案全部硬编码中文【高】
- **位置**：全仓无 `react-i18next`/`i18next`/`react-intl`；`i18n/translations.ts` 仅为智能体/工具名中文映射
- **影响**：无法切换语言；非中文用户不可用；文案改动需逐文件改。
- **建议**：引入 `react-i18next`，抽取文案为 key；至少保留中/英双语骨架。
...
#### 🟡 X-03｜无任务模板/playbook 复用机制【中】
- **建议**：提供模板库，支持常见渗透场景的快速启动。
...
#### 🔴 X-05｜安装脚本仅 bash 实现，无 Windows PowerShell 等价物【严重】
- **位置**：`setup.sh`（全文件）
- **描述**：Windows 用户必须 WSL，原生 Windows 部署阻塞；macOS 下 `set -euo pipefail` 与系统 bash 3.x 不兼容。
- **建议**：增加 `detect_os` 分支；提供 `setup.ps1`；macOS 提示用户使用 brew bash。
...
#### 🟢 X-13｜`--network host` 不支持 macOS/Windows【低】
- **位置**：`executors.py:384`
- **建议**：检测非 Linux 时改用 `--network bridge` 并提示端口映射限制。
...
| F-04 | 长会话无虚拟列表 | 前端 |
| X-05 | 安装脚本仅 bash，无 Windows 支持 | 部署 |
...
*报告结束。本次评估共识别 84 项功能缺陷（去重后），其中严重 9 项、高 26 项、中 38 项、低 11 项。所有发现均基于只读源码审查，未修改任何文件。*
