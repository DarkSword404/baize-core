# 白泽 (Baize) — AI 驱动的多智能体安全操作平台

> **v1.3.0** — 长期记忆（经验系统）+ 护栏（Guardrails）+ 外部接收器
>
> 白泽是一个基于大型语言模型的本地化 AI 安全助手，内置 30+ 个面向 Web 渗透、移动安全、无线/射频、红蓝对抗、应急响应与合规审计等场景的专用智能体。
...
- [配置模型](#配置模型)
- [智能体体系](#智能体体系)
- [经验系统（v1.3.0）](#经验系统v130)
- [护栏 Guardrails（v1.3.0）](#护栏-guardrailsv130)
- [API 接口](#api-接口)
- [架构](#架构)
- [目录结构](#目录结构)
- [Roadmap](#roadmap)
...
- **多智能体协同**：30+ 个专业安全智能体，按需调度。
- **LLM 无关**：兼容 OpenAI / DeepSeek / 通义千问 / 本地 Ollama 等 OpenAI 协议端点，模型可热切换。
- **工具调用（Function Calling）**：内置 10+ 安全专用工具（端口扫描、HTTP 探测、DNS 解析、文件操作等），智能体可自主调用。
- **长期记忆（经验系统，v1.3.0 新增）**：会话经验自动向量化入库，新问题自动检索命中历史经验，越用越聪明。
- **护栏 Guardrails（v1.3.0 新增）**：文件化可配置的输入/输出护栏策略，对请求与回答进行合规校验与注入防护。
- **外部接收器（v1.3.0 新增）**：Webhook / Syslog / 文件监听，打通外部事件源。
- **实时交互**：SSE 流式输出、会话持久化、前端自带美观的暗色 UI。
- **本地化部署**：所有数据保存在本地，隐私安全可控。
...
模型配置保存在用户目录 `~/.baize/model.json`（Web 界面「设置」页自动生成与管理）：
...
| `context_window` | 模型上下文窗口大小（token），用于自动推导 token 预算 |
| `max_context_tokens` | 上下文 token 预算上限（0 = 不限制） |
| `max_message_chars` | 单条消息最大字符数（0 = 不限制） |
| `enable_context_summary` | 超预算时是否用 LLM 压缩历史为摘要 |
...
经验系统为白泽提供**长期记忆**能力：把解决问题的方法沉淀为经验，并在后续对话中自动检索复用。
...
## 护栏 Guardrails（v1.3.0）

护栏以**文件化策略**的形式对模型输入/输出进行前置校验，覆盖：
...
- **新增 护栏 Guardrails**：文件化策略、注入防护、敏感信息保护、自定义规则热加载
...
本项目代码仅供安全研究、授权测试与教学使用。使用前请阅读「安全声明」。
