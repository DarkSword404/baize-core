...
export function CreateSessionModal({ open, onClose, onCreated }: Props): JSX.Element | null {
  ...
  async function handleCreate() {
    if (!scope.trim() || !goal.trim()) {
      addToast({ type: 'error', title: '信息不完整', message: '请填写目标范围和成功条件' });
      return;
    }
    setCreating(true);
    try {
      // 协作模式：传入 scope + goal，后端初始化黑板（origin/goal 节点），
      // 不再预选 agent —— 由黑板根据 Fact-Intent 图状态动态派发。
      ...
      addToast({
        type: 'success',
        title: '协作会话已创建',
        message: `目标 ${scope.trim()} · ${goal.trim()}`,
      });
      onClose();
      ...
      <div className="relative bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-slide-up">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">协作模式 · 黑板驱动 · 多智能体自主搜索</p>
          </div>
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={creating || !scope.trim() || !goal.trim()}
